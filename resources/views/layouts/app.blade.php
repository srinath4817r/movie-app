...
<head>
    ...
    <style>
        ...
        body{font-family:Arial,sans-serif;background:#111;color:white;margin:0;padding-top:140px}.content{padding:20px}.section-title{margin:30px 0 10px 10px;color:#ffcc00}.movie-row{display:flex;gap:15px;overflow-x:auto;padding-bottom:20px;padding: 10px;}.movie-row::-webkit-scrollbar{height:10px}.movie-row::-webkit-scrollbar-thumb{background:#555;border-radius:5px}
        .card{position:relative;width:150px;height:220px;border-radius:10px;overflow:visible;cursor:pointer;flex:0 0 auto;background-size:cover;background-position:center;transition:all .4s ease-in-out .6s;box-sizing:border-box}
        ...
        .people-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:20px}.person-card{background-color:#1a1a1a;border-radius:8px;overflow:hidden;text-align:center;transition:transform .3s ease,box-shadow .3s ease}.person-card:hover{transform:scale(1.05);box-shadow:0 5px 15px rgba(0,0,0,.5)}.person-card img{width:100%;height:225px;object-fit:cover;display:block}.person-card .person-name{padding:10px;font-weight:700;font-size:1rem}
        
        @media (max-width: 768px) {
            body{padding-top:70px}
            .robot-container{justify-content:space-between}.robot-container h1{font-size:1.5rem}
            body.people-page .main-nav { display: none; }
            .main-nav{display:none}
            .search-container{margin-top:0}
            body.mobile-nav-active .main-nav{display:flex;position:fixed;inset:0;background:rgba(0,0,0,.95);z-index:2000;justify-content:center;align-items:center}
            body.mobile-nav-active .main-nav ul{flex-direction:column;gap:30px}
            body.mobile-nav-active .main-nav a{font-size:2rem}
            .speech-bubble {left: auto;right: 105%;top: 50%;bottom: auto;transform: translateY(-50%) scale(0);transform-origin: right center;}
            .speech-bubble.show {transform: translateY(-50%) scale(1);}
            .speech-bubble::after {left: auto;right: -8px;top: 50%;transform: translateY(-50%);border-color: transparent transparent transparent white;border-style: solid;border-width: 8px 0 8px 8px;}
        }
    </style>
</head>
<body class="{{ Route::currentRouteName() == 'people.index' ? 'people-page' : '' }}">
    <header class="robot-container">
        <h1><span>Rotten</span><span> Tomatoes 🎬</span></h1>
    ...
    <nav class="main-nav">
        <ul>
            ...
            <li><a href="{{ route('movies.index') }}"><span>Movies</span></a></li>
            <li><a href="{{ route('tv-shows.index') }}"><span>TV Shows</span></a></li>
            <li><a href="{{ route('contact.index') }}"><span>Contact Us</span></a></li>
            <li><a href="{{ route('people.index') }}"><span>People</span></a></li>
        </ul>
    </nav>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function setRobotMode(newMode, target = null) {
                ...
                if (robotMode === 'search' && searchInput) {
            ...
            mainNav.addEventListener('click', (e) => {
                ...
                if (link && document.body.classList.contains('mobile-nav-active')) {
                    e.preventDefault();
                    const pageName = link.textContent.trim();
                    const destinationUrl = new URL(link.href);
                    destinationUrl.searchParams.set('navigated_from', pageName);
                    window.location.href = destinationUrl.toString();
                }
            });

            const urlParams = new URLSearchParams(window.location.search);
            const navigatedFrom = urlParams.get('navigated_from');
            if (navigatedFrom) {
                robot.classList.add('joyful');
                speechBubble.textContent = `We are on the ${navigatedFrom} page!`;
                speechBubble.classList.add('show');
                history.replaceState(null, '', window.location.pathname);
                setTimeout(() => {
                    robot.classList.remove('joyful');
                    speechBubble.classList.remove('show');
                }, 3000);
            }
            ...
            window.addEventListener('mousemove', (e) => {
                if (robotMode !== 'card') {
                    pupils.forEach(pupil => {
                        ...
                        pupil.style.transform = `translate(${3 * Math.cos(angle)}px, ${3 * Math.sin(angle)}px)`;
                    });
                }
